 ...
 * b2b-demo-marketplace checkout. Idempotent — safe to re-run; each change is applied only if not
 * already present.
 *
 * Two independent things:
 * 1. Grants the SeeSearchRankingRandomImpactPermissionPlugin permission to a real, loggable-in customer
 *    (search-admin@test-company.example / change123) — the account this package's own Presentation
 *    suite and README already document. The underlying customer/company-user/role rows are SHARED with
 *    the sibling search-debug/search-feedback/search-ranking-optimizer packages (same account, one role
 *    per package's own permission) — this script only ever ADDS its own permission row and glossary
 *    keys, so running it alongside any sibling's own apply.php is safe in either order.
 * 2. Drops this package's own search_ranking_metric.csv / search_ranking_product_metric.csv (bundled
 *    verbatim in this fixtures/ dir — standalone new files per FIXTURE_CLAIMS.md, no shared-file edits,
 *    no product/slot claims) into place and registers both as import actions.
 *
 ...
 *   ./docker/sdk console data:import search-ranking-metric
 *   ./docker/sdk console data:import search-ranking-product-metric
...
$demoshopRoot = rtrim($demoshopRoot, '/');
$dataDir = $demoshopRoot . '/data/import/common/common';
...
/**
 * Copies a bundled fixture CSV verbatim into the target data dir — only if the target doesn't already
 * have IDENTICAL content, so a re-run never clobbers a target-side edit silently.
 *
 * @param string $sourcePath
 * @param string $targetPath
 *
 * @return bool True if the file was written, false if it already matched.
 */
function copyFixtureCsv(string $sourcePath, string $targetPath): bool
{
    $sourceContent = (string)file_get_contents($sourcePath);

    if (is_readable($targetPath) && file_get_contents($targetPath) === $sourceContent) {
        return false;
    }
...
/**
 * Registers the two search-ranking data-import actions in a project's own import config YAML, right
 * after the last `product-abstract-store` action — only if not already present. Both
 * catalog_setup_import_config_EU.yml (used by scoped catalog-only imports) and full_EU.yml (the
 * full-shop import) need this, since either one might be the one a user actually runs.
 *
 * @param string $yamlPath
 *
 * @return bool True if the file was changed.
 */
function registerSearchRankingImportActions(string $yamlPath): bool
{
    if (!is_readable($yamlPath)) {
        return false;
    }

    $content = (string)file_get_contents($yamlPath);

    if (str_contains($content, 'search-ranking-metric')) {
        return false;
    }

    $addition = "  - data_entity: search-ranking-metric\n"
    ...
    file_put_contents($yamlPath, rtrim($content, "\n") . "\n" . $addition);
...
$permissionRowAdded = applyPermissionRow($dataDir, 'test-company_Admin', 'SeeSearchRankingRandomImpactPermissionPlugin');
...
$metricCopied = copyFixtureCsv(__DIR__ . '/search_ranking_metric.csv', $dataDir . '/search_ranking_metric.csv');
echo 'search_ranking_metric.csv: ' . ($metricCopied ? 'written' : 'already up to date') . "\n";

$productMetricCopied = copyFixtureCsv(__DIR__ . '/search_ranking_product_metric.csv', $dataDir . '/search_ranking_product_metric.csv');
echo 'search_ranking_product_metric.csv: ' . ($productMetricCopied ? 'written' : 'already up to date') . "\n";

$catalogYmlChanged = registerSearchRankingImportActions($demoshopRoot . '/data/import/common/catalog_setup_import_config_EU.yml');
echo 'catalog_setup_import_config_EU.yml: ' . ($catalogYmlChanged ? 'registered' : 'already registered') . "\n";

$fullYmlChanged = registerSearchRankingImportActions($demoshopRoot . '/data/import/local/full_EU.yml');
echo 'full_EU.yml: ' . ($fullYmlChanged ? 'registered' : 'already registered') . "\n";
...
echo "  ./docker/sdk console data:import search-ranking-metric\n";
echo "  ./docker/sdk console data:import search-ranking-product-metric\n";
