...
class CalibrationController extends AbstractController
{
     ...
     * @var string
     */
    protected const URL_CALIBRATION = '/search-ranking-gui/calibration';
    ...
    public function indexAction(Request $request)
    {
        ...
        if ($uploadForm->isSubmitted() && $uploadForm->isValid()) {
            ...
            $searchRankingFacade->createCalibration(
            ...
            $this->addSuccessMessage(
                'Calibration run uploaded — the next "search-ranking:calibrate" cron tick will calculate it.',
            );

            return $this->redirectResponse(static::URL_CALIBRATION);
        }

        $latestCalibrationTransfer = $searchRankingFacade->findLatestCalculatedCalibration();
        $currentRelevanceSaturationPoint = $searchRankingFacade->getRelevanceSaturationPoint();
