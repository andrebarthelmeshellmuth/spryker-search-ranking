...
interface ProductMetricGapFinderInterface
{
     ...
     *   row at all — a gap: this product was never assigned a raw value for this business score, one way
     *   or another (never imported, or imported then deleted). Each row: `id_product_abstract`, `sku`,
     *   `missing_metric_name`.
    ...
    /**
     * Specification:
     * - Total number of gap rows for the given metric filter (or across all active metrics when
     *   `null`), ignoring any search term — DataTables' `recordsTotal`.
     *
     * @api
     *
     * @param int|null $idSearchRankingMetric
     ...
     *   given search term — DataTables' `recordsFiltered`.
     *
     * @api
     *
     * @param int|null $idSearchRankingMetric
