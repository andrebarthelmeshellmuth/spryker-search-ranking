 ...
 */
interface SearchRankingConstants
{
    /**
     * Specification:
     * - Toggles whether the `search-ranking-widget/check-installation` Yves diagnostic page's route
     *   registers at all.
     ...
     *   in. The random-impact admin preview is worth checking this way specifically because every one of
     *   its integration steps fails silently: an unregistered result formatter, an unsynced ranking
     *   configuration, an unimported glossary and an unbuilt frontend all produce a storefront that looks
     *   fine and simply never shows the checkbox or the badges.
     * - The permission check in {@see \SprykerCommunity\Yves\SearchRankingWidget\Controller\CheckInstallationController}
     ...
     * @var string
     */
    public const IS_CHECK_INSTALLATION_PAGE_ENABLED = 'SEARCH_RANKING:IS_CHECK_INSTALLATION_PAGE_ENABLED';
}
