...
interface FunctionScoreBuilderInterface
{
     ...
     * built script when `$configurationTransfer->getBeta()` is below `1.0` — passing a vector alongside
     * `beta == 1.0` (the default) has no effect on the resulting script, which stays byte-identical to
     ...
     * beta used inside the built script to `1.0` (pure lexical), regardless of what
     * `$configurationTransfer->getBeta()` says — an identifier query wants exact lexical precision;
