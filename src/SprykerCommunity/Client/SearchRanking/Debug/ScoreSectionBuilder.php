...
class ScoreSectionBuilder implements ScoreSectionBuilderInterface
{
    ...
    public function build(
        ...
        $metricLines = $this->buildMetricLines($configurationTransfer, $documentScores, $decimalFormat);

        if ($metricLines['lines'] === []) {
            return null;
        }

        $section = [
            'title' => static::SECTION_TITLE,
            'lines' => $this->withRandomMetricLast($metricLines['lines']),
            'summaryLabel' => static::SUMMARY_LABEL,
            'summaryValue' => $metricLines['signalTotal'],
        ];
        ...
        if ($queryScore !== null && $queryScore >= 0) {
            return $this->addRelevanceLines(
                ...
                $queryScore,
                $metricLines['signalTotal'],
                $metricLines['hasActiveWeight'],
    ...
    /**
     * One line per metric ("signal × weight = contribution"), plus the running total and whether any
     * metric actually had a non-zero weight — split out of {@see build()} purely to keep that method's
     * own size/complexity down, no behavioral change.
     *
     * @param \Generated\Shared\Transfer\SearchRankingConfigurationStorageTransfer $configurationTransfer
     * @param array<string, float> $documentScores
     * @param string $decimalFormat
     *
     * @return array{lines: array<int, array<string, mixed>>, signalTotal: float, hasActiveWeight: bool}
     */
    protected function buildMetricLines(
        SearchRankingConfigurationStorageTransfer $configurationTransfer,
        array $documentScores,
        ...
        // gates ONLY {@see addRelevanceLines()}'s `formulaCalculation` — the one field that actively
        // CLAIMS to reproduce the real final `_score`; every other field (business-signal lines/total,
        // saturation point, normalized relevance, relevance weight) stays a true statement about the
        // CONFIGURED values and the query's own real text-relevance score either way, function_score or
        // not.
        ...
        return ['lines' => $lines, 'signalTotal' => $signalTotal, 'hasActiveWeight' => $hasActiveWeight];
    }

    /**
     * Adds the saturation-point/normalized-relevance/relevance-weight lines, and — only when
     * $hasActiveWeight — the closing formula, onto $section. Split out of {@see build()} purely to keep
     * that method's own size/complexity down, no behavioral change.
     *
     * @param array<string, mixed> $section
     * @param \Generated\Shared\Transfer\SearchRankingConfigurationStorageTransfer $configurationTransfer
     * @param float $queryScore
     * @param float $signalTotal
     * @param bool $hasActiveWeight
     * @param \Generated\Shared\Transfer\SearchRankingSpecificityWeightingResultTransfer|null $specificityWeightingResult
     * @param string $decimalFormat
     *
     * @return array<string, mixed>
     */
    protected function addRelevanceLines(
        array $section,
        SearchRankingConfigurationStorageTransfer $configurationTransfer,
        float $queryScore,
        float $signalTotal,
        bool $hasActiveWeight,
        ?SearchRankingSpecificityWeightingResultTransfer $specificityWeightingResult,
        string $decimalFormat,
    ): array {
        ...
        // reproducible-by-eye against the real final score. See SearchRankingSpecificityWeightingResultTransfer's own docblock.
        $relevanceWeight = $specificityWeightingResult !== null
            ? $specificityWeightingResult->getRelevanceWeightOrFail()
    ...
    public function buildSpecificitySection(SearchRankingSpecificityWeightingResultTransfer $specificityWeightingResult): array
    {
        ...
        // not carried on SearchRankingSpecificityWeightingResultTransfer, since it's trivially re-derivable from
        // normalizedSpecificity alone and doesn't need its own snapshot field.
        $signedDeviation = (2 * $specificityWeightingResult->getNormalizedSpecificityOrFail()) - 1;
        ...
        return [
            ...
            // Deliberately stops after the shift line, without its own "Effective relevance weight (α)"
            // line — that exact same number (getRelevanceWeight()) already renders right below this
            // collapsible section, OUTSIDE the fold, as build()'s own RELEVANCE_WEIGHT_LABEL line (see
            // this class's own top docblock for where it sits). Repeating it inside the fold too would be
            // the same value shown twice a few pixels apart.
            'lines' => array_merge(
                $this->buildSpecificityNormalizationLines($specificityWeightingResult, $decimalFormat),
                $this->buildSpecificityShiftLines($specificityWeightingResult, $decimalFormat, $signedDeviation, $deviationSign),
            ),
        ];
    }

    /**
     * The first half of {@see buildSpecificitySection()}'s lines: the configured weight through the
     * normalized specificity — split out purely to keep that method's own size down, no behavioral
     * change.
     *
     * @param \Generated\Shared\Transfer\SearchRankingSpecificityWeightingResultTransfer $specificityWeightingResult
     * @param string $decimalFormat
     ...
     */
    protected function buildSpecificityNormalizationLines(
        SearchRankingSpecificityWeightingResultTransfer $specificityWeightingResult,
        string $decimalFormat,
    ): array {
        return [
            [
                ...
                'value' => $specificityWeightingResult->getConfiguredRelevanceWeightOrFail(),
            ],
            [
                'label' => static::SPECIFICITY_SATURATION_POINT_LABEL,
                'value' => $specificityWeightingResult->getSpecificitySaturationPointOrFail(),
            ],
            [
                'label' => static::SPECIFICITY_RAW_LABEL,
                'value' => $specificityWeightingResult->getRawSpecificityOrFail(),
            ],
            [
                ...
                'calculation' => sprintf(
                    ...
                    $specificityWeightingResult->getRawSpecificityOrFail(),
                    $specificityWeightingResult->getSpecificityCurveExponentOrFail(),
                    $specificityWeightingResult->getRawSpecificityOrFail(),
                    $specificityWeightingResult->getSpecificityCurveExponentOrFail(),
                    $specificityWeightingResult->getSpecificitySaturationPointOrFail(),
                    $specificityWeightingResult->getSpecificityCurveExponentOrFail(),
                ),
                'value' => $specificityWeightingResult->getNormalizedSpecificityOrFail(),
                'stacked' => true,
            ],
        ];
    }
    ...
    /**
     * The second half of {@see buildSpecificitySection()}'s lines: the signed deviation through the
     * shift applied to α — split out purely to keep that method's own size down, no behavioral change.
     *
     * @param \Generated\Shared\Transfer\SearchRankingSpecificityWeightingResultTransfer $specificityWeightingResult
     * @param string $decimalFormat
     * @param float $signedDeviation
     * @param int $deviationSign
     *
     * @return array<int, array<string, mixed>>
     */
    protected function buildSpecificityShiftLines(
        SearchRankingSpecificityWeightingResultTransfer $specificityWeightingResult,
        string $decimalFormat,
        float $signedDeviation,
        int $deviationSign,
    ): array {
        return [
            [
                ...
                'calculation' => sprintf(
                    '2 × ' . $decimalFormat . ' - 1',
                    $specificityWeightingResult->getNormalizedSpecificityOrFail(),
                ),
                'stacked' => true,
            ...
            [
                ...
                'value' => $specificityWeightingResult->getSpecificityWeightShiftMagnitudeOrFail(),
            ],
            [
                ...
                // buildSpecificitySection()'s own comment) rather than folding the signed value into the
                ...
                'calculation' => sprintf(
                    ...
                    $specificityWeightingResult->getSpecificityWeightShiftMagnitudeOrFail(),
                    $deviationSign,
                    abs($signedDeviation),
                    $specificityWeightingResult->getSpecificityWeightExponentOrFail(),
                ),
                'value' => $specificityWeightingResult->getShiftOrFail(),
            ],
        ];
    }
